...
Structure:
\begin{lstlisting}
ROB[ROB_DEPTH]  -- flat buffer, depth = 256
Each slot:
    valid     1 bit
    data    512 bits
    status    4 bits
Free list: 256 entries
...
Depth: \texttt{ROB\_DEPTH = 256}. Implemented as dual-port SRAM/BRAM. Watermark threshold: \texttt{ROB\_DEPTH - ROB\_WATERMARK = 256 - 128 = 128}.

\textbf{Merge Logic.}
Merge Logic is a purely combinational block that sits between the ROB and the R Response Generator. It has no internal storage and introduces no buffering. Its responsibilities are:
\begin{itemize}[noitemsep]
  \item \textbf{RID recovery:} Extracts \texttt{RID[5:0] = AXID[5:0]} from the ROB-emitted beat. Per-AXID ordering is already guaranteed by the ROB head pointer scheme — Merge Logic does not perform any reordering.
  \item \textbf{RLAST generation:} Asserts \texttt{RLAST} when \texttt{rob\_last} is received coincident with the final data beat. \texttt{rob\_last} arrives as a sideband signal from the RTT in parallel with the ROB data beat — Merge Logic combines both in the same cycle.
  \item \textbf{Status passthrough:} Forwards \texttt{status[3:0]} from the ROB to the R Response Generator unchanged. The R Response Generator maps non-zero status to \texttt{DECERR} on the AXI R channel.
  \item \textbf{Data passthrough:} Forwards \texttt{data[511:0]} from the ROB to the R Response Generator unchanged.
\end{itemize}
Merge Logic does not arbitrate between AXIDs — the ROB Emit Logic's priority encoder already selects one AXID per cycle. Merge Logic sees a single beat per cycle at most and forwards it directly. No inter-AXID buffering or sequencing is required here.
...
\begin{lstlisting}
ROB occupancy >= ROB_DEPTH - ROB_WATERMARK  (= 256 - 128 = 128)
...
\end{lstlisting}
The \texttt{ROB\_WATERMARK = 128} headroom ensures all in-flight requests already accepted by MC Core can return completions without overflowing the ROB. This value is derived conservatively from MC Core's confirmed maximum pipeline depth of 100, rounded up to account for scheduler speculation and forward pressure.
% ─────────────────────────────────────────────
...
The ROB depth is sized to ensure the ROB never overflows given MC Core's maximum in-flight pipeline depth. \texttt{MC\_RD\_BUF = 100} is MC Core's confirmed maximum pipeline depth. \texttt{ROB\_DEPTH = 256} is chosen as the next power of two above \texttt{2 $\times$ MC\_RD\_BUF}, providing margin for scheduler speculation and forward pressure. Since \texttt{RESP\_READY} must always be 1, CIF cannot stall incoming completions. Instead, CIF halts new read request issuance (\texttt{PKT\_READY = 0}) when ROB occupancy reaches \texttt{ROB\_DEPTH - ROB\_WATERMARK = 128}. The \texttt{ROB\_WATERMARK = 128} headroom absorbs all completions in-flight within MC Core without ROB overflow.
